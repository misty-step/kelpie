// Lucide icons + deterministic workspace identity (icon + hue from name hash).
import { h } from './dom.js';

// ------------------------------------------------------------
// Lucide icons — inner SVG markup (24x24 viewBox, stroke-based)
// lucide-static@0.525.0, ISC licence
// ------------------------------------------------------------
const LUCIDE = {
  ...
  "hammer": '<path d="m15 12-8.373 8.373a1 1 0 1 1-3-3L12 9" /><path d="m18 15 4-4" /><path d="m8 8 6-6" /><path d="m9 7 8 8" /><path d="m21 11-8-8" />',
  ...
  "waves": '<path d="M2 6c.6.5 1.2 1 2.5 1C7 7 7 5 9.5 5S12 7 14.5 7 17 5 19.5 5c1.3 0 1.9.5 2.5 1" /><path d="M2 12c.6.5 1.2 1 2.5 1C7 13 7 11 9.5 11s2.5 2 5 2 2.5-2 5-2c1.3 0 1.9.5 2.5 1" /><path d="M2 18c.6.5 1.2 1 2.5 1C7 19 7 17 9.5 17s2.5 2 5 2 2.5-2 5-2c1.3 0 1.9.5 2.5 1" />',
};

// Workspaces churn constantly, so identity is derived, never configured:
// hash the workspace name into a fixed icon + hue vocabulary.
const ICON_POOL = [
  "rocket", "anchor", "compass", "box", "cpu", "ghost", "globe", "key", "map",
  "moon", "mountain", "origami", "palette", "puzzle", "radar", "sailboat", "telescope", "turtle",
  "leaf", "cat", "waypoints", "landmark", "skull", "flask-conical", "bird", "wrench", "hammer",
  "flower-2", "brain", "git-branch", "database", "users", "shield", "gavel", "notebook-pen",
];

// 18-step hue vocabulary for deterministic color derivation.
const HUE_STEPS = [210, 25, 340, 160, 45, 280, 190, 0, 130, 320, 75, 245, 15, 195, 55, 295, 110, 230];
...
function iconFor(wsName) {
  return ICON_POOL[hashStr(wsName) % ICON_POOL.length];
}

function hueFor(wsName) {
  return HUE_STEPS[hashStr(wsName) % HUE_STEPS.length];
}

function svgIcon(name, size) {
  const body = LUCIDE[name] || LUCIDE['box'];
  const s = size || 18;
  return '<svg xmlns="http://www.w3.org/2000/svg" width="' + s + '" height="' + s + '" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">' + body + '</svg>';
}

// Build an avatar element for a workspace name.
// size: 'sm' (28px), '' (36px default), 'lg' (44px)
function avatarEl(wsName, size) {
  const icon = iconFor(wsName);
  const hue = hueFor(wsName);
  const cls = 'avatar' + (size ? ' avatar-' + size : '');
  const el = h('div', { class: cls, html: svgIcon(icon, size === 'sm' ? 15 : size === 'lg' ? 22 : 18) });
  el.style.setProperty('--avatar-hue', hue);
  return el;
}

export { svgIcon, avatarEl, iconFor, hueFor };
